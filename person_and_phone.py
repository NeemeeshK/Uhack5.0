...
def draw_outputs(img, outputs, class_names):
    ...
    print('boxes',boxes,'objectness',objectness,'classes',classes,'nums',nums)
...
def personCount(outputs,class_names):
    ...
    for i in range(nums):
        if class_names[int(classes[i])]=='person':
            ...
            personCnt+=1
        elif class_names[int(classes[i])]=='cell phone':
            cellCnt+=1
    return personCnt, cellCnt, persons

def person_and_cell_count(image):
    yolo = YoloV3()
    load_darknet_weights(yolo, 'PersonDetection/models/yolov3.weights') 
    img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    img = cv2.resize(img, (320, 320))
    img = img.astype(np.float32)
    img = np.expand_dims(img, 0)
    img = img / 255
    class_names = [c.strip() for c in open("PersonDetection/models/classes.TXT").readlines()]
    boxes, scores, classes, nums = yolo(img)
    return personCount((boxes,scores, classes, nums),class_names)
